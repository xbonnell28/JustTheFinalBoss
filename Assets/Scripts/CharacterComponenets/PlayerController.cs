...
public class PlayerController : MonoBehaviour
{
	...
	private void Update()
	{
        ...
        {
            jumpBufferCounter = jumpBufferTime; // Need to reset timer when jump pressed while in air
        }
        jumpingHeld = Input.GetButton("Jump");
		attackCheck();
	}
	...
	private void movementCheck()
	{
		...
		if(isGrounded)
		{
			...
			Debug.Log("Ground acceleration: " + accelRate);
		}
		else
		{
			...
			Debug.Log("Air acceleration: " + accelRate);
		}
	...
	private bool ShortHopCheck()
	{
		return rb.velocity.y > 0 
			&& (!jumpingHeld || currentJumpTime > maxHeldJumpTimer)
			|| wasOnWall;
	}
